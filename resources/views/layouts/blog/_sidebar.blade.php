    ...
    <div class="pr25 pl25 clearfix">
        <form action="#">
            <div class="blog-sidebar-form-search">
                <input type="text" name="search" class="" placeholder="e.g. Javascript" value="{{request('searcch')}}">
    ...
    <div class="mt25 pr25 pl25 clearfix">
        ...
        <ul class="blog-sidebar pl25">
            @foreach($categories as $category)
                <li class="active">
                    <a href="{{route('blog.category',$category->id)}}">{{$category->name}}<span class="badge badge-pasific pull-right">{{$category->posts()->published()->get()->count()}}</span></a>
                </li>
            @endforeach
        </ul>
